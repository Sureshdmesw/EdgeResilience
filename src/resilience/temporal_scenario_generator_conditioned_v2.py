...
def evolve_state(state, rng, future=False):
    ...
    if future:
        ...
        current_pressure = clamp(
            0.45 * cyber_pressure(state["cyber"])
            + 0.30 * connectivity_pressure(state["connectivity"])
            + 0.15 * vehicle_pressure(state["vehicle"])
            + 0.10 * (1.0 - state["integrity"])
        )

        # Future-event probability depends partly on the observed state.
        # The future event itself remains stochastic and separately generated.
        event_probability = clamp(
            0.05 + 0.45 * current_pressure
        )
...
def generate_temporal_scenario(index, rng):
    ...
    return {
        "scenario_id":
            f"ER_TEMPORAL_CONDITIONED_V2_{index:06d}",
...
if __name__ == "__main__":
    ...
    print("=" * 90)
    print("EDGERESILIENCE CONDITIONED TEMPORAL GENERATOR V2")
    print("=" * 90)
    ...
    print("FUTURE_EVENT_PROBABILITY = STATE_CONDITIONED")
