...
def cyber_pressure(cyber):
    return clamp(
        0.25 * cyber["message_rate_norm"]
        + 0.20 * cyber["timing_norm"]
        + 0.25 * cyber["payload_norm"]
        + 0.15 * cyber["hamming_norm"]
        + 0.15 * cyber["id_entropy_pressure"]
    )
...
def generate_initial_state(rng):
    ...
    cyber = {
        "message_rate_norm": rng.uniform(0.05, 0.95),
        "timing_norm": rng.uniform(0.05, 0.95),
        "payload_norm": rng.uniform(0.05, 0.95),
        "hamming_norm": rng.uniform(0.05, 0.95),
        "id_entropy_pressure": rng.uniform(0.05, 0.95),
    }
...
def evolve_state(state, rng, future=False):
    ...
    # Future evolution includes a controlled stochastic stress event.
    if future:

        event_probability = rng.uniform(
            0.05,
            0.35,
        )
        ...
        if rng.random() < event_probability:
            ...
            if event_type == "CYBER":
                cyber["message_rate_norm"] = clamp(
                    cyber["message_rate_norm"]
                    + rng.uniform(0.15, 0.35)
                )
                cyber["timing_norm"] = clamp(
                    cyber["timing_norm"]
...
if __name__ == "__main__":
    ...
    print("=" * 90)
    print(
        "EDGERESILIENCE TEMPORAL SCENARIO GENERATOR"
    )
    print("=" * 90)
    ...
    print(
        "TARGET_IS_CURRENT_STATE_FORMULA = False"
    )

    print(
        "FUTURE_STATE_GENERATED_SEPARATELY = True"
    )

    print(
        "TEMPORAL_SCENARIO_GENERATION = PASS"
    )
